...
4. **Imbalance handling, tested empirically** — SMOTE oversampling and `class_weight='balanced'` were both tried and compared against the untouched baseline. Neither improved on it (see Key Findings), so the baseline was kept.
5. **Threshold tuning** — swept decision thresholds from 0.1 to 0.9 to find the best precision/recall trade-off for the actual cost asymmetry (missing a fraud costs far more than one extra manual review).
...
Extending the threshold sweep with dollar estimates (using the $122.21 average fraud amount):
...
- This is a two-day snapshot from a single, undisclosed European issuer in 2013 — fraud patterns drift over time and geography, so this would need periodic retraining in production, not a one-time fit.
...
├── creditcard.csv             # Dataset (not included — download from Kaggle link above)
├── requirements.txt
└── README.md
```
...
Full execution (including the SMOTE and class-weight comparison models) takes roughly 15–20 minutes on a single CPU core, most of it spent fitting the three Random Forest variants.
